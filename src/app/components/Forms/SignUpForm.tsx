...
export const SignupForm = ({ isActive, setIsActive }: Props) => {
  ...
  return (
    <form
      className="grid items-center justify-center gap-4 bg-white p-10 rounded-2xl mx-auto transition-all duration-300 ease-in-out transform hover:scale-105 shadow-lg"
      style={{boxShadow: "rgba(0, 0, 0, 0.35) 0px 5px 15px"}}
      action="/auth/sign-up"
      aria-label="login-form"
      method="post"
    >
      <h1 className="text-center text-4xl font-semibold text-black pb-5">Hey There</h1>

      {/* Input fields: email & password  */}
      <div className="flex flex-col gap-10 pb-10">
        <div className="flex items-center">
          <input
            className="input input-ghost input-bordered w-96 rounded-full"
            name="email"
            type="email"
            value={email}
      ...
      <div className="flex flex-col gap-2">
        <button className="bg-green-700 hover:bg-green-900 text-white rounded-full px-4 py-2 text-foreground">
          Register
        </button>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="link link-hover text-xs text-green-600 hover:text-green-500"
        >
          Sign up
        </button>
      </div>
